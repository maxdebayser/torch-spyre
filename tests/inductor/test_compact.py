...
# -------- Unit test: EdgeCostMap sparse→sparse zero-cost relabel --------


def test_edge_cost_map_sparse_to_sparse_relabel_is_free():
    """EdgeCostMap recognises sparse→sparse with same host shape as zero-cost.

    A sparse stick holds one valid element + 63 garbage slots; the bytes are
    independent of which PyTorch dim the synthetic stick is glued to.  This
    test exercises the second REINTERPRET flavour in
    EdgeCostMap._compute_and_cache_cost directly, locking in zero cost for
    the metadata-only relabel that lets spyre::reinterpret accept a sparse
    producer regardless of which PyTorch dim the producing reduction picked
    as its stick.
    """
    from types import SimpleNamespace

    from torch_spyre._C import SpyreTensorLayout
    from torch_spyre._inductor.optimize_restickify import REINTERPRET, EdgeCostMap

    # Two sparse STLs over the same host shape, distinguished by which
    # PyTorch dim carries the synthetic stick.
    host_size = (128, 192)
    host_stride = [192, 1]
    in_stl = SpyreTensorLayout(host_size, host_stride, torch.float16, [1, 0, -1])
    target_stl = SpyreTensorLayout(host_size, host_stride, torch.float16, [0, 1, -1])

    assert int(in_stl.stride_map[-1]) == -1  # in_stl is sparse
    assert int(target_stl.stride_map[-1]) == -1  # target_stl is sparse
    assert in_stl != target_stl
    assert in_stl.element_arrangement == target_stl.element_arrangement

    # Construct EdgeCostMap without going through __init__ (which would need
    # a live V.graph).  _compute_and_cache_cost only reads the attrs set here
    # for this code path; the restickify fallback at the bottom is unreached.
    ecm = EdgeCostMap.__new__(EdgeCostMap)
    ecm._dep_layout = SimpleNamespace(size=host_size, dtype=torch.float16)
    ecm._cost = {in_stl: {}}
    ecm._layout = {in_stl: {}}

    ecm._compute_and_cache_cost(in_stl, target_stl)

    assert ecm._cost[in_stl][target_stl] == 0.0, (
        "Expected sparse→sparse relabel to be zero cost"
    )
    assert ecm._layout[in_stl][target_stl] is REINTERPRET, (
        "Expected REINTERPRET sentinel for sparse→sparse relabel"
    )
